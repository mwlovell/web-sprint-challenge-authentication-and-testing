...
describe('POST /register', () => {
  ...
  test('If password is not included, responds with "username and password required" and user is not created', async () => {
    res = await request(server).post('/api/auth/register').send({ username: 'jommmmy' })
    expect(res.body.message).toBe('username and password required')
    const users = await db('users')
    expect(users).toHaveLength(3)
  })
  test('If username is already taken, responds with "username taken" and user is not created', async () => {
    const res = await request(server).post('/api/auth/register').send({ username: 'jimbo', password: '1234' })
  ...
  test('Hashed (bcrypted) password is saved to database, not plaintext', async () => {
    const password = '1234'
    await request(server).post('/api/auth/register').send({ username: 'jommmmy', password })
  ...
  test('On successful register, creates a new username in the database', async () => {
    await request(server).post('/api/auth/register').send({ username: 'jim', password: '1234' })
    const users = await db('users')
    const user = await db('users').where('id', 4).first()
    expect(user).toMatchObject({ username: 'jim', id: 4 })
    expect(users).toHaveLength(4)
  })
  test('On successful register, responds with 201 status', async () => {
    let res = await request(server).post('/api/auth/register').send({ username: 'jim', password: '1234' })
...
describe('POST /login', () => {
  ...
  test('If username is not in database, responds with "invalid credentials"', async () => {
    const res = await request(server).post('/api/auth/login').send({ username: 'jimth', password: '1234' })
    expect(res.body.message).toBe("invalid credentials")
  })
  test('if password is incorrect, responds with status 401', async () => {
    const res = await request(server).post('/api/auth/login').send({ username: 'jimbo', password: '4321' })
    expect(res.status).toBe(401)
  })
  ...
  test('on successful login, returns status 200', async () => {
    const res = await request(server).post('/api/auth/login').send({ username: 'jimbo', password: '1234' })
